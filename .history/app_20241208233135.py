...
def save_memory(memory):
    """Save persistent memory to a JSON file."""
...
st.write("Engage with the AI in your preferred style. It fetches real-time data when necessary and remembers your instructions across sessions.")

# Display current instructions
st.write(f"**Current Instructions:** {persistent_memory['instructions']}")
...
if submit_button:
    if user_input.strip():
        with st.spinner("Generating response..."):
            try:
                # Check if the user is providing new instructions
                if user_input.lower().startswith("act as"):
                    # Update persistent memory with new instructions
                    persistent_memory['instructions'] = user_input
                    save_memory(persistent_memory)
                    st.write("Instructions updated.")
                else:
                    ...
                    # Invoke the chain with instructions and user query
                    result = chain.invoke({
                        "instructions": persistent_memory['instructions'],
                        "query": user_input
                    })
