...
def fetch_google_news(query: str) -> str:
    """
    Fetches the latest news articles related to the query from Google News RSS feed.
    """
    rss_url = f"https://news.google.com/rss/search?q={query}"
    try:
        feed = feedparser.parse(rss_url)
        if feed.entries:
            # Return the title and link of the first article
        ...
        with st.spinner("Generating response..."):
            try:
                ...
                if should_fetch_real_time_data(user_input):
                    real_time_info = fetch_google_news(user_input)
