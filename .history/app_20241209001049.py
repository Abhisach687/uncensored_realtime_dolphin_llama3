...
def fetch_news_via_api(query: str) -> str:
    """
    Fetches the latest news articles related to the query using the NewsData.io API.
    """
    api_key = 'pub_61727d2bbf17888aa27b731dd77cb573aa21a'  # Replace with your NewsData.io API key
...
def fetch_news_via_scraping(query: str) -> str:
    ...
    try:
        ...
        # Extract the first news article title and link
        article = soup.find('div', class_='BNeawe vvjwJb AP7Wnd')
        link = soup.find('a', href=True)
        if article and link:
            return f"{article.text}: {link['href']}"
        else:
            return "No relevant news articles found."
    except Exception as e:
        return f"An error occurred while fetching news data: {e}"
...
def fetch_real_time_news(query: str) -> str:
    """
    Attempts to fetch real-time news using the API first; falls back to web scraping if necessary.
    """
    api_result = fetch_news_via_api(query)
    if "An error occurred" in api_result:
